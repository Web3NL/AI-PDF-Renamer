...
class MetadataExtractor:
    ...
    def _prepare_image_content(self, images: List[Image.Image], pdf_processor, max_pages: int = None) -> List[Dict[str, str]]:
        """Convert images to base64 format for API submission"""
        image_parts = []
        # Limit images to reduce API costs
        num_images = max_pages if max_pages is not None else min(len(images), DEFAULT_MAX_PAGES)
    ...
    def _parse_api_response(self, response, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._create_fallback_response(filename, response.text, f"Unexpected error: {str(e)}")

    def _create_fallback_response(self, filename: str, raw_response: str, error: str = None) -> Dict[str, Any]:
    ...
    def extract_metadata_from_images(
        ...
        for attempt in range(max_retries):
            try:
                image_parts = self._prepare_image_content(images, pdf_processor, max_pages)
                ...
                if should_retry and attempt < max_retries - 1:
                    ...
                    return self._create_error_response(
                        f"{error_type} exceeded after {max_retries} attempts: {error_message}", filename
                    )
                else:
                    return self._create_error_response(f"API call failed: {error_message}", filename)

        return self._create_error_response(f"Failed after {max_retries} attempts", filename)